...
func NewCache(objMaxCount int32) (cache *ObjectCache) {
	cache = &ObjectCache{
...
func NewDefaultCache() (cache *ObjectCache) {
	return NewCache(0)
}

//set 缓存字符切片为键值的对象，当存储的类型不匹配返回错误
// key为键值；obj为存储对象；expireSecond为过期时间（单位是秒），如果为0则不过期
func (c *ObjectCache) Set(key []byte, obj interface{}, expireSecond int) (ok bool) {
...
// key为键值；obj为存储对象；expireSecond为过期时间（单位是秒），如果为0则不过期
func (c *ObjectCache) SetInt(key int64, obj interface{}, expireSecond int) (ok bool) {
	var bKey [8]byte
...
func (c *ObjectCache) Get(key []byte) (obj interface{}, ok bool) {
	hashVal := internal.HashFunc(key)
	segID := hashVal % storage.MaxSegmentSize
...
//GetInt 根据int型键值获取对象
//ok 为是否获取成功，false则说明cache里面已经不存在此对象（可能被淘汰或者被Del()函数删除）
func (c *ObjectCache) GetInt(key int64) (obj interface{}, ok bool) {
	var bKey [8]byte
	binary.LittleEndian.PutUint64(bKey[:], uint64(key))
	return c.Get(bKey[:])
}
...
func (c *ObjectCache) Del(key []byte) (ok bool) {
	hashVal := internal.HashFunc(key)
	segID := hashVal % storage.MaxSegmentSize
	var n *internal.Node
...
// DelInt 根据int型键值删除对象
// ok返回为false则说明对象删除前已经不存在
func (c *ObjectCache) DelInt(key int64) (ok bool) {
	var bKey [8]byte
	binary.LittleEndian.PutUint64(bKey[:], uint64(key))
	return c.Del(bKey[:])
}
...
func (c *ObjectCache) GetObjCount() (count int32) {
	return c.controller.GetTotalCount()
}


// GetQueueCount 测试使用
func (c *ObjectCache) GetQueueCount() (result string) {
